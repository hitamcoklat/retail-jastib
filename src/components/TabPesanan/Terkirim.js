...
import { TouchableOpacity, ActivityIndicator, View, Text, StyleSheet, ToastAndroid, FlatList, Dimensions } from 'react-native';
...
export default function Terkirim() {
    ...
    const fetchData = () => {
        ...
        return fetch(API_URL + '/api/getPesanan?id_kustomer=' + user.ID + '&status=terkirim&token=' + user.TOKEN + '&page=' + pageNumber + '&limit=10')
    ...
    const _loadMoreData = (page) => {
        ...
        fetch(API_URL + '/api/getPesanan?id_kustomer=' + user.ID + '&status=terkirim&token=' + user.TOKEN + '&page=' + page + '&limit=10')
            .then((response) => response.json())
            .then((res) => {
            ...
            <View key={`item_${item.ID}`} style={{ marginBottom: 20 }}>
                <View style={{
                    ...
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                        ...
                        <Text style={{ fontSize: 16, color: '#515151', fontWeight: 'bold', textTransform: 'uppercase', color: 'green' }}>{item.STATUS}</Text>
                    </View>
                    <TouchableOpacity
